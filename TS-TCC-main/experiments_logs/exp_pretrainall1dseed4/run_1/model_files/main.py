...
if training_mode == "self_supervised" and data_type == 'SEED':
    ...
    train_dl, valid_dl, test_dl = data_generator_free(root_path, configs, training_mode, trial, args.ratio)
...
def train(lr,weight_decay):
    ...
    # Trainer
    best_model_loss, best_model_acc = Trainer(model, temporal_contr_model, model_optimizer, temporal_contr_optimizer, train_dl, valid_dl, test_dl, device, logger, configs, experiment_log_dir, training_mode)
...
if training_mode != "self_supervised":
    ...
    with open(f'out_{training_mode}_acc_{args.scale_channel}_{args.ratio}.txt', 'a') as f:
    ...
    with open(f'out_{training_mode}_acc_{args.scale_channel}_{args.ratio}_best_loss.txt', 'a') as f:
        f.write(f'best_acc{loss_acc}\n') 
        f.close()
    if trial == 15:
        with open(f'out_{training_mode}_acc_{args.scale_channel}_{args.ratio}_best_loss.txt', 'a') as f:
            f.write('\n') 
    ...
    with open(f'out_{training_mode}_acc_{args.scale_channel}_{args.ratio}_best_acc.txt', 'a') as f:
        f.write(f'best_acc{acc_acc}\n') 
        f.close()
    if trial == 15:
        with open(f'out_{training_mode}_acc_{args.scale_channel}_{args.ratio}_best_acc.txt', 'a') as f:
            f.write('\n') 
