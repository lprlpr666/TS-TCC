...
from dataloader.dataloader import data_generator,data_generator_all,data_generator_all_seed_34
from trainer.trainer import Trainer, model_evaluate
from models.TC import TC
from utils import _calc_metrics, copy_Files
from models.model import base_Model,Conv2d_model,Conv1d_single_Model,Conv1d_single2_Model, Conv1d_single3_Model, Conv_seperate_model
import warnings
...
parser.add_argument('--id', default='1', type=str,
                    help='SEED id')
parser.add_argument('--less_finetune', default='F', type=str)
...
trial = int(args.id)
if data_type == 'SEED' and args.less_finetune == 'F':
    data_path = f"../autodl-tmp/SEED-Dataset/SEED/{trial}/"
elif data_type == 'SEED' and args.less_finetune == 'T':
    data_path = f"/userhome2/dairenjie/TimeSeries/data/SEEDperSESSION/{trial}/"

if training_mode == "self_supervised":
    train_dl, valid_dl, test_dl = data_generator_all_seed_34(configs, training_mode)
else:
...
def train(lr,weight_decay):
    # Load Model
    # model = Conv2d_model(configs).to(device)
    # model = base_Model(configs).to(device)
    model = Conv1d_single3_Model(configs).to(device)
    # model = Conv_seperate_model(configs).to(device)
...
if training_mode != "self_supervised":
    acc = train(0.001, 0.01)
    with open(f'out_{training_mode}_acc.txt', 'a') as f:
        f.write(f'best_acc{acc}\n') 
        f.close()
