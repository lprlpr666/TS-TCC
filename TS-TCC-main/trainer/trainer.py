...
def Trainer(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device, logger, config, experiment_log_dir, training_mode):
    ...
    if training_mode != "self_supervised":  # no need to run the evaluation for self-supervised mode.
        ...
        logger.debug('\nEvaluate on the Test set:')
        test_loss, test_acc, prc, _, _ = model_evaluate(model, temporal_contr_model, test_dl, device, training_mode)
        logger.debug(f'Test loss      :{test_loss:0.4f}\t | Test Accuracy      : {test_acc:0.4f} | Test AUPRC      : {prc:0.4f}')
        # with open(f'out_{training_mode}.txt', 'a') as f:
        #     f.write(f'Test loss      :{test_loss:0.4f}\t | Test Accuracy      : {test_acc:0.4f} | Test AUPRC      : {prc:0.4f}\n') 
        #     f.close()

    weight = model.get_weight()
    logger.debug("\n################## Training is Done! #########################")
    return best_model_loss, best_model_acc, weight
...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config, device, training_mode):
    ...
    for batch_idx, (data, labels, aug1, aug2) in enumerate(train_loader):
        ...
        # labels_brain_area = [i for i in range(4) for _ in range(16)]
        # labels_brain_area = np.array(labels_brain_area)
        # labels_brain_area = torch.from_numpy(labels_brain_area).long().to(device)
        ...
        if training_mode == "self_supervised":
            ...
            # temp_cont_loss1, temp_cont_lstm_feat1 = temporal_contr_model(features1, features2, labels_brain_area)
            # temp_cont_loss2, temp_cont_lstm_feat2 = temporal_contr_model(features2, features1, labels_brain_area)
...
def model_evaluate(model, temporal_contr_model, test_dl, device, training_mode):
    ...
    with torch.no_grad():
        for data, labels, _, _ in test_dl:
            ...
            if training_mode != "self_supervised":
                ...
                onehot_label = F.one_hot(labels, 8)
    ...
    if training_mode == "self_supervised":
        total_acc = 0
        return total_loss, total_acc, total_prc, [], []
    else:
        total_acc = torch.tensor(total_acc).mean()  # average acc
    return total_loss, total_acc, total_prc, outs, trgs
